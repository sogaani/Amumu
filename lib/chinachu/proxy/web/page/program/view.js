    ...
    draw: function () {
        ...
        if (program._isRecorded) {
            ...
            var onFileSuccess = function (t, e) {
                ...
                // 録画済みサムネイル
                var imgurl = "./api/recorded/" + program.id + "/preview.jpg?width=480&height=270" + (e ? ("&encoded=" + e) : "");
            ...
            new Ajax.Request('./api/recorded/' + program.id + '/file.json', {
                ...
                onFailure: function (t) {
                    new Ajax.Request('./api/recorded/' + program.id + '/file.json?encoded=org', {
                        method: 'get',
                        requestHeaders: ["If-Modified-Since", "Thu, 01 Jun 1970 00:00:00 GMT"],
                        onSuccess: (t) => {onFileSuccess(t,"org")},
                        onFailure: function (t) {
                            ...
                            if (t.status === 410) {
                                var alert = new sakura.ui.Alert({
                                    type: 'red',
                                    body: 'この番組のオリジナルファイルは移動または削除されています',
                                    disableClose: true
                                });
