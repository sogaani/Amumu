(async function () {
	...
	if (program === null || !request.query.encoded || request.query.encoded === '') {
		if (request.method === "GET" || request.method === "HEAD") {
			...
			response.setHeader('Location', request.url.replace(/\/recorded\//, '/trans_to_chinachu/recorded/'));
			response.head(301);
			response.end();
			return;
		}
	...
	switch (request.method) {
		...
		case 'DELETE':
			...
			await db.removeEncoded(program, program.encoded[request.query.encoded]);
